...
func (r *mysqlUserRepository) Create(ctx context.Context, user *entity.User) error {
	...
	query := `INSERT INTO users (id, username, email, password_hash, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())`
...
func (r *mysqlUserRepository) GetByID(ctx context.Context, id string) (*entity.User, error) {
	query := `SELECT id, username, email, password_hash, is_active, last_login_at, created_at, updated_at FROM users WHERE id = ?`
	user := &entity.User{}
...
func (r *mysqlUserRepository) GetByUsername(ctx context.Context, username string) (*entity.User, error) {
	query := `SELECT id, username, email, password_hash, is_active, last_login_at, created_at, updated_at FROM users WHERE username = ?`
	user := &entity.User{}
	var lastLoginAt sql.NullTime
...
func (r *mysqlUserRepository) GetByEmail(ctx context.Context, email string) (*entity.User, error) {
	query := `SELECT id, username, email, password_hash, is_active, last_login_at, created_at, updated_at FROM users WHERE email = ?`
	user := &entity.User{}
	var lastLoginAt sql.NullTime
...
func (r *mysqlUserRepository) Update(ctx context.Context, user *entity.User) error {
	query := `UPDATE users SET username = ?, email = ?, password_hash = ?, is_active = ?, updated_at = NOW() WHERE id = ?`
...
func (r *mysqlUserRepository) AssignRole(ctx context.Context, userID, roleID string) error {
	query := `INSERT IGNORE INTO user_roles (user_id, role_id, created_at) VALUES (?, ?, NOW())`
